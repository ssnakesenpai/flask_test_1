...
original_author="ssnakesenpai"
original_name="flask_test_1"
original_urlname="flask_test_1"
original_description="Awesome flask_test_1 created by ssnakesenpai" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flask_test_1 "${name}"
...
then
    ...
    echo "then run 'flask_test_1' or 'python -m flask_test_1'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flask_test_1' or 'python -m flask_test_1' or 'poetry run flask_test_1'"
fi
